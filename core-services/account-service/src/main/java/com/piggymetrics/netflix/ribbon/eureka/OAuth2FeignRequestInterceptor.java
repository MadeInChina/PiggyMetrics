package com.piggymetrics.netflix.ribbon.eureka;
...
/**
 * Pre-defined custom RequestInterceptor for Feign Requests. It uses the {@link OAuth2ClientContext
 * OAuth2ClientContext} provided from the environment and construct a new header on the request
 * before it is made by Feign.
 *
...
public class OAuth2FeignRequestInterceptor implements RequestInterceptor {
  ...
  /**
   * Fully customizable constructor for changing token type and header name, in cases of Bearer and
   * Authorization is not the default such as "bearer", "authorization".
   *
  ...
  /**
   * Extracts the token extract id the access token exists or returning an empty extract if there is
   * no one on the context it may occasionally causes Unauthorized response since the token extract
   * is empty.
   *
   ...
   * @throws UserRedirectRequiredException in case the user needs to be redirected to an approval
   *     page or login page
   */
